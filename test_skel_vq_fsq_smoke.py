"""Smoke test SkelVQ with FSQ L=8 quantizer."""
import sys
...
print(f"Quantizer: {model.quantizer_type}, levels={opt.fsq_levels}")
...
print(f"loss_quant: {loss_dict['loss_quant'].item():.4f}  (FSQ has no aux loss -> 0)")
print(f"fsq_diag (mean abs(q)): {loss_dict['fsq_diag'].item():.4f}")

recon_loss = ((out - x) ** 2).mean()
total = recon_loss + loss_dict["loss_quant"]
total.backward()
...
print("\n[OK] SkelVQ-FSQ L=8 smoke test passed.")
